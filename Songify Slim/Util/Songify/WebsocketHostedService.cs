    ...
    public class WebsocketHostedService : IHostedService
    {
        ...
        private async Task ChannelPointsCustomRewardRedemptionAdd(object sender, ChannelPointsCustomRewardRedemptionArgs args)
        {
            ...
            if (Settings.Settings.TwRewardId.Any(o => o == reward.Id))
            {
                ...
                if (!Settings.Settings.TwSrUnlimitedSr && TwitchHandler.MaxQueueItems(redemption.UserName, userlevel.Max()))
                {
                    ...
                    response = response.Replace("{maxreq}", $"{(TwitchUserLevels)userlevel.Max()} {TwitchHandler.GetMaxRequestsForUserLevel(userlevel.Max())}");
